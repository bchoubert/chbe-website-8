      ...
      <div key={p.name}>
        <IconWithText
          ...
          rightContent={p.link ? (
            <Link passHref href={p.link}>
              <a className="d_link_icon" style={{ color: common.color }} target="_blank">
        ...
        <div className="d_tech">
          {(p.technologies || []).map((tech, i) => (
            <Pill key={`p_${i}`} icon={tech.icon} light={common.light} title={tech.title} />
          ))}
        </div>
...
const EDetails: FC<EDetailsProps> = ({
  work,
  experience,
}) => {
  const common: IBusinessCommon = useMemo(() => work || experience, [work, experience]);
  ...
  return (
    <>
      <div className="EDetails">
        {details.company && <EDetailsCompany details={details} common={common} />}
        {details.role && <EDetailsRole details={details} common={common} />}
        {details.products && <EDetailsProducts details={details} common={common} />}
        {details.sub && <ESubDetails subDetails={details} common={common} />}
      </div>
      <style jsx global>{`
      .EDetails .d_title {
